...
intervening null byte will not terminate the string; The given string,
including the null byte, is appended along with the implicit trailing null byte.
...
You cannot use a label inside an array expression, both simple and compound:
...
- **`no_std` compatible**: The generated bye slice is static, and does not
